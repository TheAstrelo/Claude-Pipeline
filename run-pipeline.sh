...
run_claude() {
  ...
  echo -e "  ${DIM}Spawning claude -p subprocess...${NC}"

  # Run Claude as a separate process — memory resets between invocations
  echo "$prompt" | claude -p --dangerously-skip-permissions 2>&1 | tee "$output_file.raw"

  # If the subprocess wrote the artifact file directly, use it
  if [[ -f "$output_file" ]]; then
    echo -e "  ${GREEN}Artifact written by subprocess${NC}"
    return 0
  fi

  # Fallback: if Claude didn't write the file, use raw output
  if [[ -f "$output_file.raw" ]]; then
    cp "$output_file.raw" "$output_file"
    echo -e "  ${YELLOW}Using raw output as artifact${NC}"
  fi
}
...
Run build, run tests, verify behavior matches design. Append results to $ARTIFACTS/qa-report.md with a ## Quality Behavior section."
      ;;
    10)
      local build_report=""
      [[ -f "$ARTIFACTS/build-report.md" ]] && build_report=$(cat "$ARTIFACTS/build-report.md")
...
run_phase() {
  ...
  run_claude "$prompt" "$ARTIFACTS/$artifact"
...
Address all HIGH and consensus issues. Write the revised design to $ARTIFACTS/design.md."
    ...
    run_claude "$prompt" "$ARTIFACTS/design.md"
...
Add steps for any MISSING requirements. Keep existing steps. Write updated plan to $ARTIFACTS/plan.md."
    ...
    run_claude "$prompt" "$ARTIFACTS/plan.md"
