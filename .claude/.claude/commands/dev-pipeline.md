...
After each artifact-producing phase (1-8), **pause and present the output to the user**. Wait for the user to say "continue" (or provide feedback) before proceeding. QA phases (9-16) run automatically without pauses.
...
touch "$SESSION_DIR/pipeline-state.jsonl"
```
...
4. **WAIT for user response** (phases 1-8)

---

## Phase 1: Research

Skip if `--skip-research` flag is passed. If skipped, emit:
```bash
echo '{"phase":1,"name":"Research","status":"skipped","verdict":null,"timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","artifact":null}' >> "$SESSION_DIR/pipeline-state.jsonl"
```
...
Investigate libraries, APIs, prior art, and competing approaches for this task. Search the existing codebase for related implementations. WebSearch for best practices and alternatives. WebFetch official docs.

Write output to $SESSION_DIR/research.md with sections:
...
Every claim must cite a URL or codebase file path. Compare at least 2 approaches."

echo '{"phase":1,"name":"Research","status":"running","verdict":null,"timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","artifact":null}' >> "$SESSION_DIR/pipeline-state.jsonl"
echo "$PROMPT" | claude -p --dangerously-skip-permissions 2>&1 | tee "$SESSION_DIR/research.md.raw"
[[ ! -f "$SESSION_DIR/research.md" ]] && [[ -f "$SESSION_DIR/research.md.raw" ]] && cp "$SESSION_DIR/research.md.raw" "$SESSION_DIR/research.md"
VERDICT=$(head -5 "$SESSION_DIR/research.md.raw" | grep -oE '(SUFFICIENT|NEEDS_MORE_RESEARCH)' | head -1); [ -z "$VERDICT" ] && VERDICT="UNKNOWN"
echo '{"phase":1,"name":"Research","status":"complete","verdict":"'"$VERDICT"'","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","artifact":"research.md"}' >> "$SESSION_DIR/pipeline-state.jsonl"
```

**Validators** (run via Grep):
```
has_recommendation   → grep -iE "## Recommendation" $SESSION_DIR/research.md
has_sources          → grep -c "Source:" $SESSION_DIR/research.md >= 1
no_research_gap      → ! grep "NEEDS_MORE_RESEARCH" $SESSION_DIR/research.md  (SOFT)
```
...
Standalone command: /research

[Summary: key findings, recommendation, sources consulted]

Artifact: {session-dir}/research.md

Tip: You can run this phase independently with `/research <task>`.

Review the research above. Reply with feedback to revise, or "continue" to proceed to Phase 2: Requirements.
```
...
RESEARCH=$(cat "$SESSION_DIR/research.md" 2>/dev/null || echo "")
...
echo '{"phase":2,"name":"Requirements","status":"running","verdict":null,"timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","artifact":null}' >> "$SESSION_DIR/pipeline-state.jsonl"
...
VERDICT=$(head -5 "$SESSION_DIR/brief.md.raw" | grep -oE '(CLEAR|NEEDS_INPUT)' | head -1); [ -z "$VERDICT" ] && VERDICT="UNKNOWN"
echo '{"phase":2,"name":"Requirements","status":"complete","verdict":"'"$VERDICT"'","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","artifact":"brief.md"}' >> "$SESSION_DIR/pipeline-state.jsonl"
```
...
```
Phase 2 Complete — Requirements crystallized.
...
Review the brief above. Reply with feedback to revise, or "continue" to proceed to Phase 3: Technical Design.
```
...
PROMPT="You are the Test Writer Agent. Generate test cases from this implementation plan.
...
Critique (edge cases to cover):
$CRITIQUE

Read existing test files to understand test patterns. For each plan step, generate unit tests, integration tests, and edge case tests.

Write output to $SESSION_DIR/test-plan.md with:
## Verdict: [READY | INCOMPLETE]
## Test Summary (table: Type | Count | Files)
## Test Cases
### Unit Tests (table: # | Description | File | Function Under Test | Input | Expected Output)
### Integration Tests (table: # | Description | File | Endpoint | Method | Status | Body)
### Edge Case Tests (table: # | Critique Issue | Test Description | File)
## Test Files Created
## Compilation Check
## Coverage Gaps
...
**Validators** (run via Grep):
```
has_test_cases     → grep -c -iE "test case|it should|describe\(|test\(" $SESSION_DIR/test-plan.md >= 1  (HARD)
covers_plan_steps  → grep -ci "step" $SESSION_DIR/test-plan.md >= 1  (SOFT)
has_coverage_goal  → grep -iE "coverage|critical path|edge case" $SESSION_DIR/test-plan.md  (SOFT)
```
...
Artifact: {session-dir}/test-plan.md
...
```bash
PLAN=$(cat "$SESSION_DIR/plan.md" 2>/dev/null || echo "No plan available")
TEST_PLAN=$(cat "$SESSION_DIR/test-plan.md" 2>/dev/null || echo "")
...
Scan for: N+1 queries, unbounded queries (no LIMIT), missing indexes, memory leaks (addEventListener/setInterval without cleanup), synchronous blocking in API routes, bundle size regressions.

Append findings to $SESSION_DIR/qa-report.md with:
## Performance Profile
**Verdict:** [PASS | WARN | FAIL]
### N+1 Query Detection (table: File | Line | Pattern | Severity | Fix)
### Unbounded Queries (table: File | Line | Query | Issue | Fix)
### Missing Indexes (table: Table | Column | Query File | Recommendation)
### Memory Leak Risks (table: File | Line | Pattern | Risk)
### Bundle Size Impact (table if frontend changed)
...
Scan for: TODO/FIXME/HACK/TEMP/XXX comments, 'as any' casts, hardcoded values, magic numbers, design deviations, missing error handling.

Append findings to $SESSION_DIR/qa-report.md with:
## Tech Debt Report
**Verdict:** [CLEAN | DEBT_LOGGED]
### Debt Items (table: # | File | Line | Type | Description | Severity | Suggested Fix | Effort)
### Design Deviations (table: Design Requirement | Implementation | Gap | Impact)
...
```bash
BUILD_REPORT=$(cat "$SESSION_DIR/build-report.md" 2>/dev/null || echo "No build report")
PLAN=$(cat "$SESSION_DIR/plan.md" 2>/dev/null || echo "")

PROMPT="You are the Rollback Planner Agent. Generate a rollback plan for this build.
...
Write output to $SESSION_DIR/rollback-plan.md with:
## Risk Level: [LOW | MEDIUM | HIGH | IRREVERSIBLE]
## Quick Rollback (git revert command)
## Detailed Steps (revert code, DB rollback SQL, cache invalidation, API compatibility)
...
- **Skip Phase 1 (Research)** if: the user passes `--skip-research`. Emit skipped JSONL (see Phase 1 section).
- **Skip Phase 2 (Requirements)** if: the user says requirements are clear, OR passes `--skip-arm`. Session setup still runs. If skipped, emit:
  ```bash
