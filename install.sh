...
if [ "$choice" = "7" ]; then
    ...
    # Copy the CONTENTS of source into target (note the trailing "/.").
    # Plain `cp -r "$source_dir" "$target_dir"` nests a full copy INSIDE an
    # existing target — that is how a stray .claude/.claude/ gets created.
    mkdir -p "$target_dir"
    cp -r "$source_dir/." "$target_dir/"
fi
